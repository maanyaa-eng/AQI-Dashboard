...
def load_environment_data():
    return pd.read_csv("environment_data_small.csv")

env_df = load_environment_data()

env_df["Env_ReceiveDateTime"] = pd.to_datetime(env_df["Env_ReceiveDateTime"], errors="coerce")
env = env_df.copy()
...
def load_status_data():
    return pd.read_csv("status_data_small.csv")

status_df = load_status_data()

status_df["StatusDateTime"] = pd.to_datetime(status_df["StatusDateTime"],errors="coerce")
...
@st.cache_data
def load_alert_data():
    return pd.read_csv("alert_data_small.csv")
...
# Gas Sensor Data
df = pd.read_csv("gas_data_small.csv")


df["Gas_ReceiveDateTime"] = pd.to_datetime(df["Gas_ReceiveDateTime"],errors="coerce")
